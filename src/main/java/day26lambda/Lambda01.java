...
public class Lambda01 {
    ...
    public static void printDistinctElementsLengthLessThanFive(List<String>myList){
        ...
        myList.stream().filter(t-> t.length()<5).distinct().forEach(Utils::printInTheSameLineWithSpace);
    ...
    public static List<String> getElementLengthMoreThanFiveWithUpperCase(List<String>myList){
        return myList.
                stream().
                filter(t-> t.length()>5).
                map(String::toUpperCase). // class name :: method name ==> method referance
    ...
    public static void printElementsLengthLessThanFiveUniqueLowerCase(List<String> myList){
        ...
        myList.stream().filter(t-> t.length()<5).distinct().map(String::toLowerCase).sorted().forEach(Utils::printInTheSameLineWithSpace);
    ...
    public static void printElementsUniqueToUpperCaseSorted(List<String> myList){
        ...
        myList.stream().distinct().map(String::toUpperCase).sorted().forEach(Utils::printInTheSameLineWithSpace);
    }
    ...
    public static void printElementsUniqueToUpperCaseSorted88(List<String> myList){
        ...
        myList.
                ...
                sorted(Comparator.comparing(String::length)).// Uzunlukları karşılaştırark yazdırdık Comparator.reverseOrder küçükten büyüğe
                forEach(Utils::printInTheSameLineWithSpace);
